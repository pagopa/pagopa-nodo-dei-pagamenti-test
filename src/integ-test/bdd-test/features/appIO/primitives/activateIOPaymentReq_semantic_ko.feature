Feature:  semantic checks for activateIOPaymentReq
...
"""
  ...
  Scenario: Check PPT_PSP_SCONOSCIUTO error on non-existent psp
    ...
    When psp sends SOAP activateIOPaymentReqq to nodo-dei-pagamenti
  ...
  Scenario: Check PPT_PSP_DISABILITATO error on disabled psp 
    Given idPSP with INT_NOT_ENABLED in activateIOPaymentReq
  ...
  Scenario: Check PPT_AUTENTICAZIONE error on password not associated to psp channel
    Given idChannel with 70000000001_01 in activateIOPaymentReq
  ...
  Scenario: Check PPT_AUTORIZZAZIONE error on psp broker not associated to psp
    ...
    When psp sends SOAP activateIOOaymentReq to nodo-dei-pagamenti
    Then check outcome is KO of activateIOPaymentReq response
    And check faultCode is PPT_AUTORIZZAZIONE of activateIOPaymentReqq response
  ...
  Scenario Outline: Check PPT_STAZIONE_INT_PA_SCONOSCIUTA error on non-existent station
    Given fiscalCode with 77777777777 in activateIOOaymentReq
    And noticeNumber with <value> in activateIOOaymentReq
    When psp sends SOAP activateIOOaymentReq to nodo-dei-pagamenti
    Then check outcome is KO of activateIOOaymentReq response
    And check faultCode is PPT_STAZIONE_INT_PA_SCONOSCIUTA of activateIOOaymentReq response
    Examples:
      | value              | soapUI test                                            |
  ...
  Scenario: Check PPT_INTERMEDIARIO_PA_DISABILITATO error on disabled pa broker
    Given fiscalCode with 77777777777 in activateIOPaymentReq
    And noticeNumber with 088456789012345678 in activateIOPaymentReq
    When psp sends SOAP activateIOPaymentReq to nodo-dei-pagamenti
    Then check outcome is KO of activateIOPaymentReq response
