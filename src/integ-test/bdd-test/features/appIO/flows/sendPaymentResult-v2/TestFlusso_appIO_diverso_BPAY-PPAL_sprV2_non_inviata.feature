Feature:  flow check for sendPaymentResult-v2 request - pagamento con appIO diverso da BPAY e PPAL, spr-v2 non inviata [T_SPR_V2_01]
   ...
   Scenario: Execute activateIOPayment request
      ...
      And checks the value $activateIOPaymentResponse.paymentToken of the record at column PAYMENT_TOKEN of the table POSITION_ACTIVATE retrived by the query generic_select with where condition NOTICE_ID = '311$iuv' and PA_FISCAL_CODE= '#creditor_institution_code#' on db nodo_online under macro generic_queries
