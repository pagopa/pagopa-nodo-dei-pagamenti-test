...
Scenario: Check sendPaymentOutcome response after nodoInoltroEsitoCarta, and check correctness of database tables
    ...
    And checks the value PAYING, PAYMENT_SENT, PAYMENT_ACCEPTED, NOTICE_GENERATED of the record at column STATUS of the table POSITION_PAYMENT_STATUS retrived by the query payment_status on db nodo_online under macro AppIO
    ...
    And verify 0 record for the table POSITION_SUBJECT retrived by the query position_subject2 on db nodo_online under macro AppIO