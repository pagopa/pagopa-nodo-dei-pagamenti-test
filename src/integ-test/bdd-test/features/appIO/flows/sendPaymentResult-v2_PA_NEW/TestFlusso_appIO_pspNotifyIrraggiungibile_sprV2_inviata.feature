Feature:  flow check for sendPaymentResult-v2 request - pagamento con appIO diverso da BPAY e PPAL, pspNotifyRes irraggiungibile, spr-v2 inviata [T_SPR_V2_05]
    ...
    Scenario: DB check1
        ...
        Then verify 5 record for the table POSITION_PAYMENT_STATUS retrived by the query payment_status on db nodo_online under macro AppIO
...
# implementare i check su RE per verificare che la sendPaymentResult-v2 inviata sia corretta (sul campo blob)
