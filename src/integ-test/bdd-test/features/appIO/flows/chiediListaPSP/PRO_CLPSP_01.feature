...
Scenario: Execute activateIOPayment (Phase 2)
    ...
    And execution query getPspCarte_new to get value on the table ELENCO_SERVIZI_PSP, with the columns COUNT(*) under macro AppIO with db name nodo_offline
    And through the query getPspCarte_new retrieve param sizeCarte at position 0 and save it under the key sizeCarte
    And execution query getPspCarte_new to get value on the table ELENCO_SERVIZI_PSP, with the columns ID under macro AppIO with db name nodo_offline
    And through the query getPspCarte_new retrieve param listaCarte at position -1 and save it under the key listaCarte
