Feature: parkedList checks
    ...
    Scenario: RPT_ERRORE_INVIO_A_PSP (part 2)
        ...
        And replace pa content with #creditor_institution_code# content
        And replace noticeNumber content with $1carrello content
        And checks the value RPT_RICEVUTA_NODO, RPT_ACCETTATA_NODO, RPT_PARCHEGGIATA_NODO, RPT_INVIATA_A_PSP_, RPT_ERRORE_INVIO_A_PSP of the record at column STATO of the table STATI_RPT retrived by the query rpt_stati_pa on db nodo_online under macro Mod1
