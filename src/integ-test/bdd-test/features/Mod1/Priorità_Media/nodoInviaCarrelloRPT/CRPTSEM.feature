Feature: process tests for nodoInviaCarrelloRPT[CRPTSEM]
    ...
    @runnable
    Scenario Outline: Execute nodoInviaCarrelloRPT request
        ...
        When PSP sends SOAP nodoInviaCarrelloRPT to nodo-dei-pagamenti
        Then Then check faultCode is <error> of nodoInviaCarrelloRPT response
        Examples:
            ...
            | identificativoDominio                 | 66666666665          | PPT_SEMANTICA                      | CRPTSEM18   |
            | identificativoUnivocoVersamento       | iuv                  | PPT_SEMANTICA                      | CRPTSEM19   |
            | codiceContestoPagamento               | CCD0111111           | PPT_SEMANTICA                      | CRPTSEM20   |
            
            #################
            |                                       |                      |                                    | CRPTSEM21   |
            | codiceContestoPagamento               | Empty                | PPT_AUTORIZZAZIONE                 | CRPTSEM23   |



            #FLUSSI#################
            | password          | pwdpwdpwdpwdpwdp | PPT_SINTASSI_EXTRAXSD | CRPTSEM12 |
            | identificativoPSP | None             | PPT_SINTASSI_EXTRAXSD | CRPTSEM13 |
            | identificativoPSP | Empty            | PPT_SINTASSI_EXTRAXSD | CRPTSEM14 |

            #DOPPIA PRIMITIVA################
            | codiceContestoPagamento | None | PPT_RPT_DUPLICATA | CRPTSEM22 |

            #no errore#########################
            | codiceContestoPagamento | aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa | PPT_SINTASSI_EXTRAXSD | CRPTSEM24 |
    ...
    Scenario Outline: Execute nodoInviaCarrelloRPT request
        ...
        And <tag> with <tagvalue> in nodoInviaRT
        When PSP sends SOAP nodoInviaCarrelloRPT to nodo-dei-pagamenti
        Then Then check faultCode is PPT_INTERMEDIARIO_PA_DISABILITATO of nodoInviaCarrelloRPT response