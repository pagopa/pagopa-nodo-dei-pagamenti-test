Feature: T123_ChiediListePSP_noPoste
  ...
  Scenario: Execution nodoChiediInfoPag
    ...
    # Carte
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns COUNT(*) under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param sizeCarte at position 0 and save it under the key sizeCarte
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns ID under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param listaCarte at position -1 and save it under the key listaCarte
    # Conto
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns COUNT(*) under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param sizeConto at position 0 and save it under the key sizeConto
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns ID under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param listaConto at position -1 and save it under the key listaConto
    # Altro
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns COUNT(*) under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param sizeAltro at position 0 and save it under the key sizeAltro
    And execution query getPsp_noPoste to get value on the table ELENCO_SERVIZI_PSP, with the columns ID under macro Mod1 with db name nodo_offline
    And through the query getPsp_noPoste retrieve param listaAltro at position -1 and save it under the key listaAltro
