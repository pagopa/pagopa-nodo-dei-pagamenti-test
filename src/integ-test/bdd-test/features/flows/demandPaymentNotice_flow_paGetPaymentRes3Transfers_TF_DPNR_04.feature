Feature:  flow checks for demandPaymentNoticeRequest - paGetPaymentRes 3Transfers - TF_DPNR_04
...
Background:
  ...
  Scenario: Execute demandPaymentNoticeRequest
    ...
    And check POSITION_RECEIPT_TRANSFER table of NODO_ONLINE contains 3 records for fk_position_receipt = (select id from position_receipt where notice_id = $demandPaymentNoticeResponse.notice_number)