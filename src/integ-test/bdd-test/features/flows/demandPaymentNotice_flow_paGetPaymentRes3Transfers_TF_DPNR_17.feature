Feature:  flow checks for demandPaymentNoticeRequest - paGetPaymentRes 3Transfers - broadcast = true - TF_DPNR_17
...
Background:
    ...
    #DB update step
    # UPDATE PA_STAZIONE_PA s SET s.BROADCAST = 'Y' WHERE s.FK_PA IN ('','')" - broadcast true for pa secondary
    ...
    Scenario: define a paGetPaymentRes on mock having 3 transfers in TransferList (1 per la paIntestataria, 2 per la stessa paSecondaria)
    ...
    # And check POSITION_RECEIPT_RECIPIENT table of NODO_ONLINE contains 2 records for noticeId = $demandPaymentNoticeResponse.notice_number (1 for paIntestataria and 1 for paSecondaria)
    # And check POSITION_RECEIPT_RECIPIENT_STATUS of NODO_ONLINE contains 2 records in status NOTICE_GENERATED for noticeId = $demandPaymentNoticeResponse.notice_number
    # And check POSITION_RECEIPT_XML of NODO_ONLINE contains 2 records for noticeId = $demandPaymentNoticeResponse.notice_number having recipient_pa_fiscal_code = 1paIntestataria and 1 for paSecondaria