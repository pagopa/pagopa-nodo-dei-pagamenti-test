Feature:  flow checks for mod3cancelV2 - pspNotifyPaymentV2 timeout, mod3CancelV2 when tokens expired and one payment different from PAYMENT_UNKNOWN [FLUSSO_NM1_M3CV2_07]
  ...
  Background:
   ...
   # activatePaymentNoticeV2 phase
	...
	# UPDATE NODO_ONLINE.POSITION_PAYMENT_STATUS_SNAPSHOT s SET s.STATUS = 'PAID' WHERE s.PAYMENT_TOKEN = '$activatePaymentNoticeV2Response.paymentToken'
