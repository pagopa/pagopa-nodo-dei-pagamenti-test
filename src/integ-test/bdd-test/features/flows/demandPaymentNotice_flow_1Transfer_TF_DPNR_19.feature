Feature:  flow checks for demandPaymentNoticeRequest - paGetPaymentRes 1Transfer - broadcast = false - TF_DPNR_19
...
Background:
    ...
    Scenario: define a paGetPaymentRes on mock having 1 transfer in TransferList for pa secondary
    ...
    And check transferList tag contains 1 transfer of activatePaymentNotice response
   ...
   # sendPaymentOutcome Phase outcome OK
    ...
    # And check POSITION_RECEIPT_RECIPIENT table of NODO_ONLINE contains 1 record for noticeId = $demandPaymentNoticeResponse.notice_number (1 for paIntestataria)
    # And check POSITION_RECEIPT_XML of NODO_ONLINE contains 1 record for noticeId = $demandPaymentNoticeResponse.notice_number having recipient_pa_fiscal_code = paIntestataria