Feature:  flow checks for demandPaymentNoticeRequest - paGetPaymentRes 2Transfers - broadcast = true - TF_DPNR_16
...
Background:
    ...
    #DB update step
    # UPDATE PA_STAZIONE_PA s SET s.BROADCAST = 'Y' WHERE s.FK_PA IN ('','')" - broadcast true 1 per la pa intestataria e 2 per la secondaria
    ...
    Scenario: define a paGetPaymentRes on mock having 2 transfers in TransferList
    ...
    And check transferList tag contains 2 transfers of activatePaymentNotice response
    ...
    # And check POSITION_RECEIPT_RECIPIENT table of NODO_ONLINE contains 3 records for noticeId = $demandPaymentNoticeResponse.notice_number (1 for paIntestataria and 2 for paSecondaria)
    # And check POSITION_RECEIPT_RECIPIENT_STATUS of NODO_ONLINE contains 3 records in status NOTICE_GENERATED for noticeId = $demandPaymentNoticeResponse.notice_number
    # And check POSITION_RECEIPT_XML of NODO_ONLINE contains 3 records for noticeId = $demandPaymentNoticeResponse.notice_number having recipient_pa_fiscal_code = 1paIntestataria and 2 for paSecondaria
