Feature: idempotency checks for sendPaymentOutcomeV2
   ...
   Scenario: IDMP_SPO_18 (part 1)
      ...
      When PSP sends SOAP activateIOPayment to nodo-dei-pagamenti
      Then check outcome is OK of activateIOPayment response
      And saving activateIOPayment request in activateIOPaymentReq
      And save activateIOPayment response in activateIOPaymentResponse
      And checks the value NotNone of the record at column ID of the table IDEMPOTENCY_CACHE retrived by the query # idempotency_aip on db nodo_online under macro NewMod1
   ...
   Scenario: IDMP_SPO_18 (part 4)
      ...
      When PSP sends SOAP sendPaymentOutcomeV2 to nodo-dei-pagamenti
      Then check outcome is OK of sendPaymentOutcomeV2 response
      And checks the value None of the record at column ID of the table IDEMPOTENCY_CACHE retrived by the query # idempotency_aip on db nodo_online under macro NewMod1
...
#              "# idempotency_aip" : "SELECT columns FROM table_name WHERE IDEMPOTENCY_KEY = '$activateIOPaymentReq.idempotencyKey'",
...
#              "noticeid_pa_token": "SELECT columns FROM table_name WHERE NOTICE_ID = '311$iuv' and ID_DOMINIO = '#creditor_institution_code#' AND PAYMENT_TOKEN = '$activateIOPaymentResponse.paymentToken' ORDER BY ID ASC",
