...
def step_impl(context, sender, soap_primitive, receiver):
    ...
    if 'NODOPGDB' in os.environ:
        ...
        headers = {'Content-Type': 'application/xml', 'SOAPAction': soap_primitive, 'X-Forwarded-For': '10.82.39.148',
                   'Host': 'api.dev.platform.pagopa.it:443'}  # set what your server accepts
...
def step_impl(context, job_name, seconds):
    ...
    if user_profile != None:
        ...
        nodo_response = requests.get(f"{url_nodo}-monitoring/monitoring/v1/jobs/trigger/{job_name}", headers=headers,
                                     verify=False)
...
@step(
    u"checks the value {value} of the record at column {column} of the table {table_name} retrived by the query {query_name} on db {db_name} under macro {name_macro}")
...
def step_impl(context, column, query_name, table_name, db_name, name_macro, number):
    ...
    if number == 'default_token_duration_validity_millis':
        ...
        value = (datetime.datetime.now().astimezone(pytz.timezone('Europe/Rome')) + datetime.timedelta(
            minutes=min)).strftime('%Y-%m-%d %H:%M')
        selected_query = utils.query_json(context, query_name, name_macro).replace(
            "columns", column).replace("table_name", table_name)
        exec_query = adopted_db.executeQuery(conn, selected_query)
        query_result = [t[0] for t in exec_query]
...
@step(
    "through the query {query_name} convert json {json_elem} at position {position:d} to xml and save it under the key {key}")
