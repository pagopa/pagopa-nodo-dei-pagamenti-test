Feature: Check codifica bar code AIM- OK
    ...
    # [FLUSSO_OK_barCode_AIM_aux0]
    Scenario: verifyRPT phase
        Given aim:AuxDigit with 0 in verifyRPT
    ...
    Scenario: activateRPT phase
        ...
        And aim:AuxDigit with 0 in activateRPT
    ...
    # [FLUSSO_OK_barCode_AIM_aux1]
    Scenario: aimCode_aux1
        Given aim:AuxDigit with 1 in verifyRPT
        When PSP sends verifyRPT to nodo-dei-pagamenti
        Then check esito is OK of verifyRPT response

    Scenario: activateRPT phase
        Given the verifyRPT phase scenario request executed successfully
        And aim:AuxDigit with 1 in activateRPT
        When PSP sends activateRPT to nodo-dei-pagamenti
        Then check esito is OK of activateRPT
    ...
    # [FLUSSO_OK_barCode_AIM_aux2]
    Scenario: aimCode_aux2
        Given aim:AuxDigit with 2 in verifyRPT
        When PSP sends verify to nodo-dei-pagamenti
        Then check esito is OK of verifyRPT

    Scenario: activateRPT phase
        Given the verifyRPT phase scenario request executed successfully
        And aim:AuxDigit with 2 in activateRPT
        When PSP sends activateRPT to nodo-dei-pagamenti
        Then check esito is OK of activateRPT
    ...
    # [FLUSSO_OK_barCode_AIM_aux3]
    Scenario: aimCode_aux3
        Given aim:AuxDigit with 3 in verifyRPT
        When PSP sends verify to nodo-dei-pagamenti
        Then check esito is OK of verifyRPT response

    Scenario: activateRPT phase
        Given the verifyRPT phase scenario request executed successfully
        And aim:AuxDigit with 3 in activateRPT
        When PSP sends activateRPT to nodo-dei-pagamenti
        Then check esito is OK of activateRPT
