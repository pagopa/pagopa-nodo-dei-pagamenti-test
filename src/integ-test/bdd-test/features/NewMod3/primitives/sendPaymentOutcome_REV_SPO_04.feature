Feature: process tests revisioni casi di blocco
  ...
  Scenario: Execution test
    ...
    When psp sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
    Then checks the value PAYING,FAILED of the record at column STATUS of the table POSITION_PAYMENT_STATUS retrived by the query payment_status on db nodo_online under macro NewMod3
  ...
  Scenario: Execution test
    Given the sendPaymentOutcome request scenario executed successfully
    When psp sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
    Then checks the value PAYING,INSERTED of the record at column STATUS of the table POSITION_STATUS retrived by the query payment_status on db nodo_online under macro NewMod3
