Feature: process tests for paSendRT [PSRT_30]
    ...
    Scenario: Define second sendPaymentOutcome
    ...
    Scenario: clean second paSendRt queue
        Given the Define second sendPaymentOutcome scenario executed successfully
        When job paSendRt triggered after 10 seconds
        And wait 10 seconds for expiration


    Scenario: job second paSendRt
        Given the Define second sendPaymentOutcome scenario executed successfully
        And initial XML paSendRT
            """
            <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:paf="http://pagopa-api.pagopa.gov.it/pa/paForNode.xsd">
