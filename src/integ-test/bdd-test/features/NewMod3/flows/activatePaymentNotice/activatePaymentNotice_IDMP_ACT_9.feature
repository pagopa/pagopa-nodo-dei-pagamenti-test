Feature: semantic check for activatePaymentNotice regarding idempotency
  ...
  #DB check
    ...
    And check datetime plus number of date 1 of the record at column VALID_TO in the row 1 of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache_act on db nodo_online under macro NewMod3
