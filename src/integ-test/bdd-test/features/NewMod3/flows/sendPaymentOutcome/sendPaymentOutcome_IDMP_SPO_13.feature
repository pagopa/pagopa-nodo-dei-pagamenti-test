Feature: semantic check for sendPaymentOutcomeReq regarding idempotency 1273
  ...
  Scenario: Execute sendPaymentOutcome request
    ...
    And initial XML sendPaymentOutcome
      ...
      <password>pwdpwdpwd</password>
      <idempotencyKey>#idempotency_key#</idempotencyKey>
      <paymentToken>798c6a817ed9482fa5659c45f4a25f286</paymentToken>
      <outcome>OK</outcome>
      <details>
    ...
    And checks the value $sendPaymentOutcome.idPSP of the record at column PSP_ID of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod3
    And checks the value None of the record at column NOTICE_ID of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod3
    ...
    And checks the value $sendPaymentOutcome.paymentToken of the record at column TOKEN of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod3
