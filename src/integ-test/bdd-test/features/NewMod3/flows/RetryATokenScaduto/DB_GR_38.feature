Feature: process tests for DB_GR_38
  ...
  Scenario: DB check + db update
   ...
   # DB Check POSITION_RECEIPT_RECIPIENT_STATUS
    ...
    And checks the value 90000000001 of the record at column RECIPIENT_PA_FISCAL_CODE of the table POSITION_RECEIPT_XML retrived by the query position_receipt_recipient_status on db nodo_online under macro NewMod3
    And checks the value 90000000001 of the record at column RECIPIENT_BROKER_PA_ID of the table POSITION_RECEIPT_XML retrived by the query position_receipt_recipient_status on db nodo_online under macro NewMod3
