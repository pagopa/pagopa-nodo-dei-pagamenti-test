Feature: process tests for retry a token scaduto
  ...
  Scenario: DB check
    ...
    And checks the value NotNone of the record at column transfer_date of the table POSITION_PAYMENT retrived by the query payment_status on db nodo_online under macro NewMod3
