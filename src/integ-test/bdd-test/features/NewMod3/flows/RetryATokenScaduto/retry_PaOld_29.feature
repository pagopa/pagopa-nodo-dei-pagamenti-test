Feature: process tests for retry a token scaduto
  ...
  @runnable
  Scenario: DB check [retry_PaOld_29]
    Given the Execute sendPaymentOutcome2 request scenario executed successfully
    And wait 5 seconds for expiration
    #STATI
    Then checks the value PAYING,INSERTED,PAYING,NOTICE_STORED of the record at column status of the table POSITION_STATUS retrived by the query payment_status on db nodo_online under macro NewMod3
    And checks the value PAYING,PAYING_RPT,CANCELLED,PAID_NORPT,PAID of the record at column status of the table POSITION_PAYMENT_STATUS retrived by the query payment_status on db nodo_online under macro NewMod3
    ...
    #And checks the value None, 2021-10-02 of the record at column transfer_date of the table POSITION_PAYMENT retrived by the query payment_status on db nodo_online under macro NewMod3
    ...
    And through the query rt_stati retrieve param id at position 0 in the row 0 and save it under the key fk_rptid0
    #And through the query rt_stati retrieve param id at position 1 and save it under the key fk_rptid1
    And through the query rt_stati retrieve param id at position 0 in the row 1 and save it under the key fk_rptid1
