Feature: process tests for retryAtokenScaduto 1178
  ...
  Scenario: DB update
    ...
    Then generic update through the query param_update_generic_where_condition of the table POSITION_ACTIVATE the parameter AMOUNT = '5', with where condition PA_FISCAL_CODE='$activatePaymentNotice.fiscalCode' AND NOTICE_ID='$activatePaymentNotice.noticeNumber' under macro update_query on db nodo_online
  ...
  Scenario: check position_payment [retry_PaOld_15_2]
    ...
    And checks the value 5 of the record at column amount of the table POSITION_ACTIVATE retrived by the query payment_status on db nodo_online under macro NewMod3
    ##
    ...
    And checks the value 5 of the record at column amount of the table POSITION_ACTIVATE retrived by the query payment_status_v2 on db nodo_online under macro NewMod3
    # TOKEN_UTILITY
