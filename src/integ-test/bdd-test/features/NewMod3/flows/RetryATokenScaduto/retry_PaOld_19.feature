Feature: process tests for retry a token scaduto 1183
  ...
  @runnable
  Scenario: DB check [retry_PaOld_19]
    ...
    And checks the value $iuv of the record at column creditor_reference_id of the table POSITION_PAYMENT retrived by the query payment_status_orderbydesc on db nodo_online under macro NewMod3
    ...
    #RPT_ACTIVATIONS
    And verify 1 record for the table RPT_ACTIVATIONS retrived by the query payment_token_v2 on db nodo_online under macro NewMod3