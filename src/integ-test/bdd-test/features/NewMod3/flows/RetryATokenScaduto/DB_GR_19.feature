Feature: process tests for DB_GR_19
  ...
  Scenario: DB check + db update
   ...
   # DB Check POSITION_RECEIPT_RECIPIENT_STATUS
    ...
    And checks the value 90000000001 of the record at column PA_FISCAL_CODE of the table POSITION_RECEIPT_RECIPIENT retrived by the query position_receipt_recipient_status on db nodo_online under macro NewMod3
