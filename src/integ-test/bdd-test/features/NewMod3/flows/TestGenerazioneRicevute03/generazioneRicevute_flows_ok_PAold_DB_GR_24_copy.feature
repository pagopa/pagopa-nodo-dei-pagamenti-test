Feature: process tests for generazioneRicevute [DB_GR_24]
  ...
  Scenario: Execute sendPaymentOutcome request
    ...
    Then check outcome is OK of sendPaymentOutcome response
     ...
     # test execution
  Scenario: Execution test DB_GR_24
    Given the Execute sendPaymentOutcome request scenario executed successfully
    And PSP waits 5 seconds for expiration
    #And wait 10 seconds for expiration
    Then execution query position_payment to get value on the table POSITION_RECEIPT, with the columns * under macro NewMod3 with db name nodo_online
