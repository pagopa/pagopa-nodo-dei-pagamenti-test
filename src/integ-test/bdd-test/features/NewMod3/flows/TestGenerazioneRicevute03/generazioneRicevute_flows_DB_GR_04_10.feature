Feature: process tests for generazioneRicevute
  ...
  Scenario: Execute sendPaymentOutcome (Phase 1)
    ...
    And checks the value NOTICE_GENERATED,NOTICE_SENT,NOTIFIED of the record at column STATUS of the table POSITION_RECEIPT_STATUS retrived by the query position_receipt_recipient_status on db nodo_online under macro NewMod3
