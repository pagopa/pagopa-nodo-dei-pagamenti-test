Feature: happy flow with Stand In on and channel no Stand In for NMU 1561

    Background:
        Given systems up

    # Lo scopo di questo test è verificare che il parametro opzionale standin=true non sia presente né nelle response verso il psp, né nella REQ della paSendRtV2
    ...
    Scenario: sendPaymentOutcomeV2 request
        ...
        And check payload tag standIn field not exists in $paSendRTV2
