  ...
  Scenario: check activateIOPayment2 OK 2 tokens
    ...
    And the activate scenario executed successfully
    When PSP sends SOAP activate to nodo-dei-pagamenti
    Then check outcome is OK of activate response
    And save activate response in activateResponse
  ...
  Scenario: nodoChiediInformazioniPagamento2
    ...
    When WISP sends REST GET informazioniPagamento?idPagamento=$activateResponse.paymentToken to nodo-dei-pagamenti
    Then verify the HTTP status code of informazioniPagamento response is 200
