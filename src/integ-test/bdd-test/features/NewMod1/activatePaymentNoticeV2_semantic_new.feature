Feature: semantic checks new for activatePaymentNoticeV2Request
    ...
    Scenario: semantic check 26
        ...
        # And checks the value $paGetPaymentV2.dueDate of the record at column TO_CHAR(DUE_DATE, 'yyyy-mm-dd') of the table POSITION_SERVICE JOIN POSITION_PAYMENT_PLAN ON (POSITION_PAYMENT_PLAN.FK_POSITION_SERVICE=POSITION_SERVICE.ID) JOIN POSITION_TRANSFER ON (POSITION_TRANSFER.FK_PAYMENT_PLAN=POSITION_PAYMENT_PLAN.ID) retrived by the query position_payment_plan on db nodo_online under macro NewMod1
