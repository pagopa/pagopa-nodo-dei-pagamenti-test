Feature: NMU flows PA Old con pagamento OK
    ...
    @ALL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_11 @after_1
    ...
    @ALL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_12 @after_1
    ...
    @ALL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_13 @after_1
    ...
    @ALL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_14 @after_1
    ...
    @ALL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_15 @after_1
    Scenario: NMU flow OK, FLOW con PA Old e PSP vp1, chiamate parallelo -> CloseV2 prima di response a RPT -> checkPosition con 1 nav, activateV2 -> paaAttivaRPT, nodoInviaRPT REQ, closeV2+ REQ -> aspetta lock, nodoInviaRPT RESP, closeV2 RESP -> pspNotify con additionalPaymentInformations, spo+ -> paInviaRT+, BIZ+ e SPR+ (NMU-4)
        Given generic update through the query param_update_generic_where_condition of the table STAZIONI the parameter INVIO_RT_ISTANTANEO = 'Y', with where condition OBJ_ID = '16635' under macro update_query on db nodo_cfg
        And wait 3 seconds after triggered refresh job ALL
        And from body with datatable horizontal checkPositionBody initial JSON checkPosition
            | fiscalCode                  | noticeNumber |
    ...
    # And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
    #     | column            | value                                                                                                                                                                                                                                                                       |
    #     | tipo_evento       | activatePaymentNoticeV2,paaAttivaRPT,paaAttivaRPT,activatePaymentNoticeV2,nodoInviaRPT,nodoInviaRPT,closePayment-v2,closePayment-v2,pspNotifyPayment,sendPaymentOutcome,pspNotifyPayment,sendPaymentOutcome,paaInviaRT,paaInviaRT,sendPaymentResult-v2,sendPaymentResult-v2 |
    #     | sotto_tipo_evento | REQ,REQ,RESP,RESP,REQ,RESP,REQ,RESP,REQ,REQ,RESP,RESP,REQ,RESP,REQ,RESP                                                                                                                                                                                                     |
    # And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table RE retrived by the query on db re with where datatable horizontal
    #     | where_keys         | where_values                                  |
    #     | PAYMENT_TOKEN      | $activatePaymentNoticeV2Response.paymentToken |
    #     | SOTTO_TIPO_EVENTO  | ('REQ','RESP')                                |
