Feature: idempotency tests for activatePaymentNoticeV2Request
    ...
    Scenario: IDMP_APNV2_11.1
        Given nodo-dei-pagamenti DEV has config parameter default_idempotency_key_validity_minutes set to 40
    ...
    Scenario: IDMP_APNV2_11.2
        Given nodo-dei-pagamenti DEV has config parameter default_idempotency_key_validity_minutes set to 10
        And nodo-dei-pagamenti DEV has config parameter default_token_duration_validity_millis set to 1800000
        And expirationTime with None in activatePaymentNoticeV2
        When psp sends SOAP activatePaymentNoticeV2 to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNoticeV2 response
    ...
    Scenario: IDMP_APNV2_11.3
        Given nodo-dei-pagamenti DEV has config parameter default_idempotency_key_validity_minutes set to 30
        And nodo-dei-pagamenti DEV has config parameter default_token_duration_validity_millis set to 1800000
        And expirationTime with None in activatePaymentNoticeV2
        When psp sends SOAP activatePaymentNoticeV2 to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNoticeV2 response
    ...
    Scenario: IDMP_APNV2_11.5
        Given nodo-dei-pagamenti DEV has config parameter default_idempotency_key_validity_minutes set to 2
    ...
    Scenario: IDMP_APNV2_12
        Given idPSP with Empty in activatePaymentNoticeV2
        And expirationTime with None in activatePaymentNoticeV2
        When psp sends SOAP activatePaymentNoticeV2 to nodo-dei-pagamenti
    ...
    Scenario: IDMP_APNV2_15.2 (part 1)
        Given password with pwdpwdpwf in activatePaymentNoticeV2
        And expirationTime with None in activatePaymentNoticeV2
        When psp sends SOAP activatePaymentNoticeV2 to nodo-dei-pagamenti
        Then check outcome is KO of activatePaymentNoticeV2 response
    ...
    Scenario: IDMP_APNV2_16.2 (part 2)
        ...
        Then check outcome is KO of activatePaymentNoticeV2 response
        And check faultCode is PPT_ERRORE_IDEMPOTENZA of activatePaymentNoticeV2 response
        And verify 0 record for the table POSITION_PAYMENT_STATUS retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table POSITION_PAYMENT_STATUS_SNAPSHOT retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table POSITION_STATUS retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table POSITION_STATUS_SNAPSHOT retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table POSITION_ACTIVATE retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table POSITION_PAYMENT retrived by the query select_activatev2 on db nodo_online under macro NewMod1
        And verify 0 record for the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
    ...
    Scenario: IDMP_APNV2_17 (part 1)
        ...
        And nodo-dei-pagamenti DEV has config parameter default_token_duration_validity_millis set to 1800000
        And nodo-dei-pagamenti DEV has config parameter scheduler.jobName_idempotencyCacheClean.enabled set to false
        And expirationTime with None in activatePaymentNoticeV2
        When psp sends SOAP activatePaymentNoticeV2 to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNoticeV2 response
        And checks the value NotNone of the record at column ID of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
    ...
    Scenario: IDMP_APNV2_18 (part 2)
        ...
        And checks the value NotNone,None of the record at column TOKEN of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        And checks the value NotNone,None of the record at column VALID_TO of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        And checks the value NotNone,None of the record at column HASH_REQUEST of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        And checks the value NotNone,None of the record at column RESPONSE of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        And checks the value NotNone,None of the record at column INSERTED_TIMESTAMP of the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        And verify 2 record for the table IDEMPOTENCY_CACHE retrived by the query idempotency_cache on db nodo_online under macro NewMod1
        Given nodo-dei-pagamenti DEV has config parameter default_idempotency_key_validity_minutes set to 10
