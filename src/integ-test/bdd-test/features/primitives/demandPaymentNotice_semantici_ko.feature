Feature:  semantic checks for demandPaymentNoticeReq
  ...
  Scenario: Check PPT_SERVIZIO_SCONOSCIUTO error on non-existent idSoggettoServizio
    Given idSoggettoServizio with idSoggettoServizioUnknown in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_SERVIZIO_NONATTIVO error on inactive idSoggettoServizio
    Given idSoggettoServizio with inactive idSoggettoServizio in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_VERSIONE_SERVIZIO error on idSoggettoServizio with version 1
    Given idSoggettoServizio with version 1 in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_DOMINIO_SCONOSCIUTO error on non-existent idDominio
    Given idSoggettoServizio associated to a 'cds_soggetto' not present in PA table in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_DOMINIO_DISABILITATO error on not enabled idDominio
    Given idSoggettoServizio associated to a 'cds_soggetto' present in PA table and not enabled in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_INTERMEDIARIO_PA_DISABILITATO error on not enabled idBrokerPA
    Given idSoggettoServizio associated to a 'cds_soggetto' present in PA table, associated to an idBrokerPA not enabled in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_STAZIONE_INT_PA_SCONOSCIUTA error on idStation not enabled to 4 model
    Given idSoggettoServizio associated to a 'cds_soggetto' present in PA table, associated to an idStation not enabled to 4 model in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario: Check PPT_STAZIONE_INT_PA_DISABILITATA error on idStation not enabled
    Given idSoggettoServizio associated to a 'cds_soggetto' present in PA table, associated to an idStation not enabled in demandPaymentNoticeReq
    When psp sends demandPaymentNoticeReq to nodo-dei-pagamenti
    Then check outcome is KO
