Feature: syntax checks for closePayment-v1 outcome KO - Nessun errore
...
#closePayment-v1 phase - SIN_CP_03.2
    Scenario: Execute a closePayment-v1 request having two paymentTokens 
    Given initial json closePayment-v1 request
    """
        {"paymentTokens": [
    ...
    When PM sends closePayment-v1 request to nodo-dei-pagamenti
    Then check "esito" is OK
    
# element value check - Nessun errore
Scenario Outline: Check no errors are raised
    Given <elem> with <value> in closePayment-v1 request
    When PM sends closePayment-v1 to nodo-dei-pagamenti
    Then check "esito" is OK of closePayment-v1 response
