Feature:  semantic checks for verificaBollettinoReq
  ...
  Scenario: Check PPT_AUTENTICAZIONE error on password not associated to psp channel
    ...
    # ccPost value check: codicePA not present in NODO4_CFG.CODIFICHE_PA table of nodo-dei-pagamenti db [SEM_VB_09]
   Scenario: Check PPT_SEMANTICA error on non-existent ccPost
    Given ccPost with 777777777772 in verificaBollettinoReq not present inside column CODICE_PA in NODO4_CFG.CODIFICHE_PA table of nodo-dei-pagamenti database
    When psp sends verificaBollettinoReq to nodo-dei-pagamenti
    Then check outcome is KO
  ...
  Scenario Outline: Check PPT_STAZIONE_INT_PA_SCONOSCIUTA error on non-existent station
    Given ccPost with 77777777777 in verificaBollettinoReq
