Feature: NM3 flows PA Old con concorrenza
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 -> activate -> paaAttivaRPT & spo+ in parallel mode-> KO PPT_SEMANTICA  (OLD_NM3-1A)
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 -> spo+ & activate -> paaAttivaRPT in parallel mode-> KO PPT_SEMANTICA  (OLD_NM3-1A)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                  | noticeNumber | amount | expirationTime |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 312#iuv#     | 10.00  | 2000           |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 ->  activate -> paaAttivaRPT  & spo- in parallel mode-> KO PPT_TOKEN_SCADUTO  (OLD_NM3-2A)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount | expirationTime |
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 -> spo- & activate in parallel mode-> KO PPT_TOKEN_SCADUTO  (OLD_NM3-2A)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount | expirationTime |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code_old# | 312#iuv#     | 10.00  | 2000           |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 -> activate & spo+ in parallel mode-> KO PPT_SEMANTICA  (OLD_NM3-5A)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount | expirationTime |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code_old# | 312#iuv#     | 10.00  | 2000           |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow KO, FLOW: activate -> mod3CancelV1 -> activate & spo+ in parallel mode-> KO PPT_TOKEN_SCADUTO  (OLD_NM3-5A)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount | expirationTime |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code_old# | 312#iuv#     | 10.00  | 2000           |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow OK, FLOW: activate -> nodoInviaRPT & spo- in parallel mode-> OK  (OLD_NM8-A)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount |
    ...
    Scenario: NM3 flow KO, FLOW: activate -> spo+ con paymentMethod sconosciuto & nodoInviaRPT in parallel mode-> KO PPT_SINTASSI_EXTRAXSD (OLD_NM9-A)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code_old# | 312#iuv#     | 10.00  |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow KO, FLOW: activate -> spo+ & nodoInviaRPT con tipoVersamento = 'TE' in parallel mode-> KO PPT_SINTASSI_EXTRAXSD (OLD_NM10A)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code_old# | 312#iuv#     | 10.00  |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
    ...
    Scenario: NM3 flow KO, FLOW: activate in parallel mode activate -> paaAttivaRPT-> -> KO PPT_ATTIVAZIONE_IN_CORSO (OLD_NM3-12A)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                      | noticeNumber | amount |
