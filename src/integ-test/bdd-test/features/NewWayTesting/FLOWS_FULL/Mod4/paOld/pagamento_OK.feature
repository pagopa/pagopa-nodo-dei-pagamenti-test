Feature: NM4 e MOD4 flows con PA OLD pagamento OK
    ...
    @ALL @FLOW @FLOW_FULL @NM4 @NM4PAOLD @NM4PAOLDPAGOK @NM4PAOLDPAGOK_FULL_1 @after
    Scenario: NM4 flow OK, FLOW con PA Old e PSP New: nodoChiediCatalogoServizi, nodoChiedinumeroAvviso -> paaChiediNumeroAvviso, activatePaymentNotice -> paaAttivaRPT, nodoInviaRPT, spo+ -> paaInviaRT+, BIZ+ (NM4-3)
        Given update for table STAZIONI with parameter INVIO_RT_ISTANTANEO = 'Y' on db nodo_cfg with where datatable horizontal
            ...
            | OBJ_ID     | 16635        |
        And update for table PA_STAZIONE_PA with parameter QUARTO_MODELLO = 'Y' on db nodo_cfg with where datatable horizontal
            | where_keys | where_values |
            | OBJ_ID     | 16643        |
        And update for table PA_STAZIONE_PA with parameter QUARTO_MODELLO = 'N' on db nodo_cfg with where datatable horizontal
            | where_keys | where_values |
            | OBJ_ID     | 1689962      |
