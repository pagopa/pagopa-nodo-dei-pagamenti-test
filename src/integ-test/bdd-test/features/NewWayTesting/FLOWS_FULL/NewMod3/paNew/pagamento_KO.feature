Feature: NM3 flows con pagamento fallito
    ...
    Scenario: NM3 flow KO, FLOW: verify -> paVerify activate -> paGetPayment   activate -> paGetPayment -> spoV2+ -> with 2 token ->  KO con PPT_SEMANTICA (OLD_NM3-97)
        ...
        And from $activatePaymentNoticeReq2.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $activatePaymentNoticeReq2.password xml check value #password# in position 0
        And from $activatePaymentNoticeReq2.qrCode.fiscalCode xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeReq2.qrCode.noticeNumber xml check value $activatePaymentNotice.noticeNumber in position 0
        ...
        And from $activatePaymentNoticeResp2.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp2.paymentToken xml check value $activatePaymentNoticeResponse.paymentToken in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.idTransfer xml check value 1 in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.transferAmount xml check value $activatePaymentNotice.amount in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.IBAN xml check value NotNone in position 0
        ...
        And from $sendPaymentOutcomeV2Resp.faultCode xml check value PPT_SEMANTICA in position 0
    ...
    Scenario: NM3 flow KO, FLOW: verify -> paVerify activate -> paGetPayment  activate random noticeNumber -> paGetPayment -> spoV2+ ->  KO con PPT_PAGAMENTO_DUPLICATO (OLD_NM3-98)
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column                | value                                         |
            | ID                    | NotNone                                       |
            | PA_FISCAL_CODE        | $activatePaymentNoticeRequest2.fiscalCode     |
            | NOTICE_ID             | $activatePaymentNoticeRequest2.noticeNumber   |
            | STATUS                | PAYING                                        |
            | INSERTED_TIMESTAMP    | NotNone                                       |
        ...
        And verify 1 record for the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            ...
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        # POSITION_PAYMENT_STATUS activate2
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column                | value                                         |
            | ID                    | NotNone                                       |
            | PA_FISCAL_CODE        | $activatePaymentNoticeRequest1.fiscalCode     |
            | NOTICE_ID             | $activatePaymentNoticeRequest1.noticeNumber   |
            | STATUS                | PAYING                                        |
            | INSERTED_TIMESTAMP    | NotNone                                       |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | PA_FISCAL_CODE        | $activatePaymentNoticeRequest2.fiscalCode                                                   |
            | NOTICE_ID             | $activatePaymentNoticeRequest2.noticeNumber                                                 |
            | CREDITOR_REFERENCE_ID | $paGetPayment_1.creditorReferenceId,$paGetPayment_2.creditorReferenceId                     |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | DUE_DATE              | 2021-12-31 00:00:00                       |
            | RETENTION_DATE        | None                                      |
            | AMOUNT                | $activatePaymentNoticeRequest2.amount     |
            | FLAG_FINAL_PAYMENT    | Y                                         |
            | INSERTED_TIMESTAMP    | NotNone                                   |
            | UPDATED_TIMESTAMP     | NotNone                                   |
            | METADATA              | NotNone                                   |
        ...
        And from $activatePaymentNoticeReq.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $activatePaymentNoticeReq.password xml check value #password# in position 0
        And from $activatePaymentNoticeReq.qrCode.fiscalCode xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $activatePaymentNoticeReq.qrCode.noticeNumber xml check value $activatePaymentNoticeRequest1.noticeNumber in position 0
        And from $activatePaymentNoticeReq.amount xml check value $activatePaymentNoticeRequest1.amount in position 0
        # activatePaymentNotice 1 RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_1Response.paymentToken |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key activatePaymentNoticeResp
        And from $activatePaymentNoticeResp.outcome xml check value OK in position 0
        And from $activatePaymentNoticeResp.totalAmount xml check value $activatePaymentNoticeRequest1.amount in position 0
        And from $activatePaymentNoticeResp.fiscalCodePA xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        ...
        And from $activatePaymentNoticeResp.transferList.transfer.idTransfer xml check value 1 in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.transferAmount xml check value $activatePaymentNoticeRequest1.amount in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.fiscalCodePA xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.IBAN xml check value NotNone in position 0
        And from $activatePaymentNoticeResp.creditorReferenceId xml check value $paGetPayment_1.creditorReferenceId in position 0
        # activatePaymentNotice 2 REQ
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_2Response.paymentToken |
        ...
        And from $activatePaymentNoticeReq.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $activatePaymentNoticeReq.password xml check value #password# in position 0
        And from $activatePaymentNoticeReq.qrCode.fiscalCode xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $activatePaymentNoticeReq.qrCode.noticeNumber xml check value $activatePaymentNoticeRequest2.noticeNumber in position 0
        And from $activatePaymentNoticeReq.amount xml check value $activatePaymentNoticeRequest2.amount in position 0
        # activatePaymentNotice 2 RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_2Response.paymentToken |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key activatePaymentNoticeResp
        And from $activatePaymentNoticeResp.outcome xml check value OK in position 0
        And from $activatePaymentNoticeResp.totalAmount xml check value $activatePaymentNoticeRequest2.amount in position 0
        And from $activatePaymentNoticeResp.fiscalCodePA xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        ...
        And from $activatePaymentNoticeResp.transferList.transfer.idTransfer xml check value 1 in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.transferAmount xml check value $activatePaymentNoticeRequest2.amount in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.fiscalCodePA xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.IBAN xml check value NotNone in position 0
        And from $activatePaymentNoticeResp.creditorReferenceId xml check value $paGetPayment_2.creditorReferenceId in position 0
        # paGetPayment 1 REQ
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_1Response.paymentToken |
            ...
            | ORDER BY           | DATA_ORA_EVENTO ASC                           |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key paGetPaymentReq
        And from $paGetPaymentReq.idPA xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $paGetPaymentReq.idBrokerPA xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $paGetPaymentReq.idStation xml check value #id_station# in position 0
        And from $paGetPaymentReq.qrCode.fiscalCode xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $paGetPaymentReq.qrCode.noticeNumber xml check value $activatePaymentNoticeRequest1.noticeNumber in position 0
        And from $paGetPaymentReq.amount xml check value $activatePaymentNoticeRequest1.amount in position 0
        # paGetPayment 2 RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_1Response.paymentToken |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key paGetPaymentResp
        And from $paGetPaymentResp.outcome xml check value OK in position 0
        And from $paGetPaymentResp.data.creditorReferenceId xml check value $paGetPayment_1.creditorReferenceId in position 0
        And from $paGetPaymentResp.data.paymentAmount xml check value $activatePaymentNoticeRequest1.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.transferAmount xml check value $activatePaymentNoticeRequest1.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.fiscalCodePA xml check value $activatePaymentNoticeRequest1.fiscalCode in position 0
        And from $paGetPaymentResp.data.transferList.transfer.IBAN xml check value IT45R0760103200000000001016 in position 0
        # paGetPayment 1 REQ
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            ...
            | ORDER BY           | DATA_ORA_EVENTO ASC                           |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key paGetPaymentReq
        And from $paGetPaymentReq.idPA xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $paGetPaymentReq.idBrokerPA xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $paGetPaymentReq.idStation xml check value #id_station# in position 0
        And from $paGetPaymentReq.qrCode.fiscalCode xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $paGetPaymentReq.qrCode.noticeNumber xml check value $activatePaymentNoticeRequest2.noticeNumber in position 0
        And from $paGetPaymentReq.amount xml check value $activatePaymentNoticeRequest2.amount in position 0
        # paGetPayment RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_2Response.paymentToken |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key paGetPaymentResp
        And from $paGetPaymentResp.outcome xml check value OK in position 0
        And from $paGetPaymentResp.data.creditorReferenceId xml check value $paGetPayment_2.creditorReferenceId in position 0
        And from $paGetPaymentResp.data.paymentAmount xml check value $activatePaymentNoticeRequest2.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.transferAmount xml check value $activatePaymentNoticeRequest2.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.fiscalCodePA xml check value $activatePaymentNoticeRequest2.fiscalCode in position 0
        And from $paGetPaymentResp.data.transferList.transfer.IBAN xml check value IT45R0760103200000000001016 in position 0
        # sendPaymentOutcomev2 REQ
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
    ...
    Scenario: NM3 flow KO, FLOW: verify -> paVerifyPayment activate -> paGetPayment -> updDB STATUS with CANCELLED activate -> paGetPayment -> spoV2+ paymentToken_2 -> OK spoV2+ paymentToken_1 -> KO con PPT_PAGAMENTO_DUPLICATO (OLD_NM3-127)
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | FEE                        | None,2.0                                                                                  |
            | OUTCOME                    | None,OK                                                                                   |
            | PAYMENT_METHOD             | None,creditCard                                                                           |
            | PAYMENT_CHANNEL            | NA,app                                                                                    |
            | TRANSFER_DATE              | None,NotNone                                                                              |
            | PAYER_ID                   | None,NotNone                                                                              |
            | APPLICATION_DATE           | None,NotNone                                                                              |
            | INSERTED_TIMESTAMP         | NotNone                                                                                   |
            | UPDATED_TIMESTAMP          | NotNone                                                                                   |
            | FK_PAYMENT_PLAN            | NotNone                                                                                   |
            | RPT_ID                     | None                                                                                      |
            ...
            | UPDATED_BY                 | activatePaymentNotice,sendPaymentOutcomeV2                                                |
            | TRANSACTION_ID             | None                                                                                      |
            | CLOSE_VERSION              | None                                                                                      |
            | FEE_PA                     | None                                                                                      |
            | BUNDLE_ID                  | None                                                                                      |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | PA_FISCAL_CODE        | $activatePaymentNotice.fiscalCode                                                                                                                                                                                                                                             |
            | CREDITOR_REFERENCE_ID | $paGetPayment.creditorReferenceId                                                                                                                                                                                                                                             |
            | PAYMENT_TOKEN         | $activatePaymentNotice1Response.paymentToken,$activatePaymentNotice2Response.paymentToken,$activatePaymentNotice2Response.paymentToken,$activatePaymentNotice2Response.paymentToken,$activatePaymentNotice2Response.paymentToken,$activatePaymentNotice2Response.paymentToken |
            | STATUS                | PAYING,PAYING,PAID,NOTICE_GENERATED,NOTICE_SENT,NOTIFIED                                                                                                                                                                                                                      |
            | INSERTED_BY           | activatePaymentNotice,activatePaymentNotice,sendPaymentOutcomeV2,sendPaymentOutcomeV2,sendPaymentOutcomeV2,paSendRT                                                                                                                                                           |
            | INSERTED_TIMESTAMP    | NotNone                                                                                                                                                                                                                                                                       |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | ID                 | NotNone                           |
            | PA_FISCAL_CODE     | $activatePaymentNotice.fiscalCode |
            | STATUS             | PAYING,PAYING,PAID,NOTIFIED       |
            | INSERTED_TIMESTAMP | NotNone                           |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
        ...
        And from $activatePaymentNoticeResp2.totalAmount xml check value $activatePaymentNotice.amount in position 0
        And from $activatePaymentNoticeResp2.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp2.paymentToken xml check value $activatePaymentNotice2Response.paymentToken in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.idTransfer xml check value 1 in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.transferAmount xml check value $activatePaymentNotice.amount in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp2.transferList.transfer.IBAN xml check value NotNone in position 0
        ...
        And from $paGetPaymentReq2.idStation xml check value #id_station# in position 0
        And from $paGetPaymentReq2.qrCode.fiscalCode xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $paGetPaymentReq2.qrCode.noticeNumber xml check value 302$iuv in position 0
        ...
        And from $paGetPaymentResp2.data.paymentAmount xml check value $activatePaymentNotice.amount in position 0
        And from $paGetPaymentResp2.data.transferList.transfer.transferAmount xml check value $activatePaymentNotice.amount in position 0
        And from $paGetPaymentResp2.data.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        ...
        And from $sendPaymentOutcomeV2Req.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $sendPaymentOutcomeV2Req.password xml check value #password# in position 0
        And from $sendPaymentOutcomeV2Req.paymentToken xml check value $activatePaymentNotice1Response.paymentToken in position 0
        And from $sendPaymentOutcomeV2Req.outcome xml check value OK in position 0
        ...
        And from $sendPaymentOutcomeV2Req2.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $sendPaymentOutcomeV2Req2.password xml check value #password# in position 0
        And from $sendPaymentOutcomeV2Req2.paymentToken xml check value $activatePaymentNotice2Response.paymentToken in position 0
    ...
    Scenario: NM3 flow KO, FLOW: verify -> paVerify activate with 2000 expirationTime-> paGetPayment ->  spoV2- -> KO  (OLD_NM3-145)
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key sendPaymentOutcomeV2Resp
        And from $sendPaymentOutcomeV2Resp.outcome xml check value KO in position 0
        And from $sendPaymentOutcomeV2Resp.faultCode xml check value PPT_TOKEN_SCADUTO_KO in position 0
    ...
    Scenario: NM3 flow KO, FLOW: activate -> paGetPayment  -> spo+ with random paymentToken -> KO with PPT_TOKEN_SCONOSCIUTO (OLD_NM3-26K)
        ...
        Given from body with datatable horizontal sendPaymentOutcomeBody_idempotency_full initial XML sendPaymentOutcome
            | idPSP | idBrokerPSP     | idChannel                    | password   | idempotencyKey    | paymentToken                      | outcome |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #idempotency_key# | 798c6a817ed9482fa5659c45f4a25f287 | OK      |
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is KO of sendPaymentOutcome response
    ...
    Scenario: NM3 flow KO, FLOW: activate -> paGetPayment  -> spo+ with fake idBrokerPSP -> KO with PPT_AUTORIZZAZIONE (OLD_NM3-27K)
        ...
        Given from body with datatable horizontal sendPaymentOutcomeBody_idempotency_full initial XML sendPaymentOutcome
            | idPSP | idBrokerPSP | idChannel                    | password   | idempotencyKey    | paymentToken                                | outcome |
            | #psp# | 80000000001 | #canale_ATTIVATO_PRESSO_PSP# | #password# | #idempotency_key# | $activatePaymentNoticeResponse.paymentToken | OK      |
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is KO of sendPaymentOutcome response
    ...
    Scenario: NM3 flow KO, FLOW: activate 1 -> paGetPayment -> activate 2 -> pagetPayment, spo-, activate 1 -> activate 2 (OLD_NM3-29K)
        ...
        Given from body with datatable horizontal sendPaymentOutcomeBody_full initial XML sendPaymentOutcome
            | idPSP | idBrokerPSP     | idChannel                    | password   | paymentToken                                 | outcome |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | $activatePaymentNotice2Response.paymentToken | KO      |
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is OK of sendPaymentOutcome response
        And wait 1 seconds for expiration
        # IDEMPOTENCY_CACHE activate1
        And verify 1 record for the table IDEMPOTENCY_CACHE retrived by the query on db nodo_online with where datatable horizontal
            | where_keys      | where_values                           |
            | IDEMPOTENCY_KEY | $activatePaymentNotice1.idempotencyKey |
        # IDEMPOTENCY_CACHE activate2
        And verify 0 record for the table IDEMPOTENCY_CACHE retrived by the query on db nodo_online with where datatable horizontal
            | where_keys      | where_values                           |
            | IDEMPOTENCY_KEY | $activatePaymentNotice2.idempotencyKey |
        Given saving activatePaymentNotice1 request in activatePaymentNotice
        When PSP sends SOAP activatePaymentNotice to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNotice response
        And verify the paymentToken of the activatePaymentNotice response is equals to paymentTokenPhase1
        Given saving activatePaymentNotice2 request in activatePaymentNotice
        When PSP sends SOAP activatePaymentNotice to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNotice response
        And verify the paymentToken of the activatePaymentNotice response is not equals to paymentTokenPhase2
        # POSITION_PAYMENT_STATUS
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column                | value                                                                               |
            | ID                    | NotNone                                                                             |
            | PA_FISCAL_CODE        | $activatePaymentNotice.fiscalCode                                                   |
            | CREDITOR_REFERENCE_ID | $paGetPayment.creditorReferenceId                                                   |
            | PAYMENT_TOKEN         | $paymentTokenPhase2,$paymentTokenPhase2,$activatePaymentNoticeResponse.paymentToken |
            | STATUS                | PAYING,FAILED,PAYING                                                                |
            | INSERTED_TIMESTAMP    | NotNone                                                                             |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys | where_values                        |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | FK_POSITION_PAYMENT   | NotNone                                                         |
            | CREDITOR_REFERENCE_ID | $paGetPayment.creditorReferenceId                               |
            | PAYMENT_TOKEN         | $paymentTokenPhase2,$activatePaymentNoticeResponse.paymentToken |
            | STATUS                | FAILED,PAYING                                                   |
            | INSERTED_TIMESTAMP    | NotNone                                                         |
            | UPDATED_TIMESTAMP     | NotNone                                                         |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS_SNAPSHOT retrived by the query on db nodo_online with where datatable horizontal
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | BROKER_PSP_ID              | $activatePaymentNotice.idBrokerPSP                              |
            | CHANNEL_ID                 | #canale_ATTIVATO_PRESSO_PSP#                                    |
            | AMOUNT                     | $activatePaymentNotice.amount,70.0                              |
            | FEE                        | 2,None                                                          |
            | OUTCOME                    | $sendPaymentOutcome.outcome,None                                |
            | INSERTED_BY                | activatePaymentNotice                                           |
            | UPDATED_BY                 | sendPaymentOutcome,activatePaymentNotice                        |
        ...
        And verify 0 record for the table PM_SESSION_DATA retrived by the query on db nodo_online with where datatable horizontal
            | where_keys  | where_values        |
            | ID_SESSIONE | $paymentTokenPhase2 |
        # POSITION_ACTIVATE
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column        | value                                                           |
            | PAYMENT_TOKEN | $paymentTokenPhase2,$activatePaymentNoticeResponse.paymentToken |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | su.ENTITY_UNIQUE_IDENTIFIER_VALUE | JHNDOE00A01F205N      |
            | su.FULL_NAME                      | NotNone               |
            | su.STREET_NAME                    | street                |
            | su.CIVIC_NUMBER                   | 12                    |
            | su.POSTAL_CODE                    | 89020                 |
            | su.CITY                           | city                  |
            | su.STATE_PROVINCE_REGION          | MI                    |
            | su.COUNTRY                        | IT                    |
            | su.EMAIL                          | john.doe@test.it      |
            | su.INSERTED_TIMESTAMP             | NotNone               |
            | su.UPDATED_TIMESTAMP              | NotNone               |
            | su.INSERTED_BY                    | activatePaymentNotice |
            | su.UPDATED_BY                     | activatePaymentNotice |
        ...
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values          |
            | PAYMENT_TOKEN      | $paymentTokenPhase2   |
            | TIPO_EVENTO        | activatePaymentNotice |
            | SOTTO_TIPO_EVENTO  | REQ                   |
            | ESITO              | RICEVUTA              |
            | INSERTED_TIMESTAMP | TRUNC(SYSDATE-1)      |
        ...
        And from $activatePaymentNoticeResp.totalAmount xml check value $activatePaymentNotice.amount in position 0
        And from $activatePaymentNoticeResp.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp.paymentToken xml check value $paymentTokenPhase2 in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.idTransfer xml check value 1 in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.transferAmount xml check value $activatePaymentNotice.amount in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.IBAN xml check value NotNone in position 0
        ...
        And from $sendPaymentOutcomeReq.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $sendPaymentOutcomeReq.password xml check value #password# in position 0
        And from $sendPaymentOutcomeReq.paymentToken xml check value $paymentTokenPhase2 in position 0
        And from $sendPaymentOutcomeReq.outcome xml check value KO in position 0
        # sendPaymentOutcome RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values        |
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_39
    Scenario: NM3 flow KO, FLOW: verify -> paVerify activate -> paGetPayment --> spo+ -> spo- KO con PPT_ESITO_GIA_ACQUISITO -> paSendRT (OLD_NM3-35K)
        Given from body with datatable horizontal verifyPaymentNoticeBody_noOptional initial XML verifyPaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                  | noticeNumber |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv#     |
        And from body with datatable vertical paVerifyPaymentNoticeBody_full initial XML paVerifyPaymentNotice
    ...
    Scenario: NM3 flow KO, FLOW: activate1 -> paGetPayment -> mod3CancelV2 activate2 -> paGetPayment -> spo+ paymentToken_1 -> KO con PPT_PAGAMENTO_DUPLICATO -> spo+ paymentToken_2 -> OK (OLD_NM3-44G)
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | PA_FISCAL_CODE        | $activatePaymentNotice.fiscalCode                                                                                                                                                                                                                                                                                                 |
            | NOTICE_ID             | $activatePaymentNotice.noticeNumber                                                                                                                                                                                                                                                                                               |
            | STATUS                | PAYING,CANCELLED,PAYING,PAID,NOTICE_GENERATED,NOTICE_SENT,NOTIFIED                                                                                                                                                                                                                                                                |
            | INSERTED_TIMESTAMP    | NotNone                                                                                                                                                                                                                                                                                                                           |
            | CREDITOR_REFERENCE_ID | $paGetPayment.creditorReferenceId                                                                                                                                                                                                                                                                                                 |
            ...
            | INSERTED_BY           | activatePaymentNotice,mod3CancelV2,activatePaymentNotice,sendPaymentOutcome,sendPaymentOutcome,sendPaymentOutcome,paSendRT                                                                                                                                                                                                        |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | FK_POSITION_PAYMENT   | NotNone                                                                                     |
            | INSERTED_BY           | activatePaymentNotice                                                                       |
            | UPDATED_BY            | mod3CancelV2,sendPaymentOutcome                                                             |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS_SNAPSHOT retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | INSERTED_TIMESTAMP | NotNone                                                                              |
            | INSERTED_BY        | activatePaymentNotice,mod3CancelV2,activatePaymentNotice,sendPaymentOutcome,paSendRT |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        ...
        And from $sendPaymentOutcomeReq.idChannel xml check value #canale_ATTIVATO_PRESSO_PSP# in position 0
        And from $sendPaymentOutcomeReq.password xml check value #password# in position 0
        And from $sendPaymentOutcomeReq.paymentToken xml check value $activatePaymentNotice_2Response.paymentToken in position 0
        And from $sendPaymentOutcomeReq.outcome xml check value OK in position 0
        # sendPaymentOutcome RESP2
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_2Response.paymentToken |
            | TIPO_EVENTO        | sendPaymentOutcome                            |
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_41
    Scenario: NM3 flow KO, FLOW: activate -> paGetPayment  -> spo+ -> Update idPSP, idBrokerPSP, idChannel spo -> spo+ con KO with PPT_TOKEN_SCONOSCIUTO (OLD_NM3-52K)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP     | idChannel                    | password   | fiscalCode                  | noticeNumber | amount |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv#     | 10.00  |
        And from body with datatable vertical paGetPayment_full initial XML paGetPayment
        ...
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is OK of sendPaymentOutcome response
        Given idPSP with 40000000001 in sendPaymentOutcome
        ...
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            ...
            | ORDER BY                                 | DATA_ORA_EVENTO ASC                         |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key sendPaymentOutcomeReq
        And from $sendPaymentOutcomeReq.idPSP xml check value 40000000001 in position 0
        And from $sendPaymentOutcomeReq.idBrokerPSP xml check value 40000000001 in position 0
        And from $sendPaymentOutcomeReq.idChannel xml check value 40000000001_01 in position 0
        And from $sendPaymentOutcomeReq.password xml check value #password# in position 0
        And from $sendPaymentOutcomeReq.paymentToken xml check value $activatePaymentNoticeResponse.paymentToken in position 0
        And from $sendPaymentOutcomeReq.outcome xml check value OK in position 0
        # sendPaymentOutcome RESP2
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_43
    Scenario: NM3 flow KO, FLOW: activate -> paGetPayment  -> spo+ -> Update idempotencyKey rand spo and -> spo+ con KO with PPT_ESITO_GIA_ACQUISITO (OLD_NM3-56K)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP     | idChannel                    | password   | fiscalCode                  | noticeNumber | amount |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv#     | 10.00  |
        And from body with datatable vertical paGetPayment_full initial XML paGetPayment
        ...
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is OK of sendPaymentOutcome response
        Given random idempotencyKey having #psp# as idPSP in sendPaymentOutcome
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is KO of sendPaymentOutcome response
        And check faultCode is PPT_ESITO_GIA_ACQUISITO of sendPaymentOutcome response
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_45
    Scenario: NM3 flow KO, FLOW: verify -> paVerify activate -> paGetPayment --> spo- -> spo+ e response KO con PPT_ESITO_GIA_ACQUISITO (OLD_NM3-38K)
        Given from body with datatable horizontal verifyPaymentNoticeBody_noOptional initial XML verifyPaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                  | noticeNumber |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv#     |
        And from body with datatable vertical paVerifyPaymentNoticeBody_full initial XML paVerifyPaymentNotice
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_46
    Scenario: NM3 flow KO, FLOW: activate1 -> paGetPayment -> activate2 -> paGetPayment -> spo+ paymentToken_2 -> OK -> activate1 OK -> activate2 KO con PPT_PAGAMENTO_DUPLICATO (OLD_NM3-51K)
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP     | idChannel                    | password   | fiscalCode                  | noticeNumber | amount |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv#     | 10.00  |
        And from body with datatable vertical paGetPayment_full initial XML paGetPayment
        ...
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP     | idChannel                    | password   | fiscalCode                  | noticeNumber | amount |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 302#iuv1#    | 10.00  |
        And from body with datatable vertical paGetPayment_full initial XML paGetPayment
            | outcome                     | OK                                |
        ...
        Given from body with datatable horizontal sendPaymentOutcomeBody_idempotency_full initial XML sendPaymentOutcome
            | idPSP | idBrokerPSP     | idChannel                    | password   | idempotencyKey    | paymentToken                                  | outcome |
            | #psp# | #id_broker_psp# | #canale_ATTIVATO_PRESSO_PSP# | #password# | #idempotency_key# | $activatePaymentNotice_2Response.paymentToken | OK      |
        When PSP sends SOAP sendPaymentOutcome to nodo-dei-pagamenti
        Then check outcome is OK of sendPaymentOutcome response
        Given saving activatePaymentNotice1 request in activatePaymentNotice
        When PSP sends SOAP activatePaymentNotice to nodo-dei-pagamenti
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | INSERTED_TIMESTAMP    | NotNone                                                                                                                                                                                                                               |
            | CREDITOR_REFERENCE_ID | $paGetPayment.creditorReferenceId                                                                                                                                                                                                     |
            | PAYMENT_TOKEN         | $activatePaymentNotice_2Response.paymentToken,$activatePaymentNotice_2Response.paymentToken,$activatePaymentNotice_2Response.paymentToken,$activatePaymentNotice_2Response.paymentToken,$activatePaymentNotice_2Response.paymentToken |
            | INSERTED_BY           | activatePaymentNotice,sendPaymentOutcome,sendPaymentOutcome,sendPaymentOutcome,paSendRT                                                                                                                                               |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
        ...
        And from $activatePaymentNoticeResp.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $activatePaymentNoticeResp.transferList.transfer.IBAN xml check value NotNone in position 0
        And from $activatePaymentNoticeResp.creditorReferenceId xml check value 02$iuv1 in position 0
        # paGetPayment REQ
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
            | PAYMENT_TOKEN      | $activatePaymentNotice_2Response.paymentToken |
        ...
        And from $paGetPaymentReq.idStation xml check value #id_station# in position 0
        And from $paGetPaymentReq.qrCode.fiscalCode xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $paGetPaymentReq.qrCode.noticeNumber xml check value 302$iuv1 in position 0
        And from $paGetPaymentReq.amount xml check value $activatePaymentNotice.amount in position 0
        # paGetPayment RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key paGetPaymentResp
        And from $paGetPaymentResp.outcome xml check value OK in position 0
        And from $paGetPaymentResp.data.creditorReferenceId xml check value 02$iuv1 in position 0
        And from $paGetPaymentResp.data.paymentAmount xml check value $activatePaymentNotice.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.transferAmount xml check value $activatePaymentNotice.amount in position 0
        And from $paGetPaymentResp.data.transferList.transfer.fiscalCodePA xml check value $activatePaymentNotice.fiscalCode in position 0
        And from $paGetPaymentResp.data.transferList.transfer.IBAN xml check value IT45R0760103200000000001016 in position 0
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_47
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PANEW @NM3PANEWPAGKO @NM3PANEWPAGKO_FULL_48
