Feature: NM3 flows PA Old con pagamento KO
    ...
    Scenario: NM3 flow KO, FLOW con PA Old e PSP vp1: verify -> paaVerificaRPT -> activate con body sbagliato con KO response -> paaAttivaRPT  nodoInviaRPT spo+ -> paaInviaRT+ (subito, senza job paInviaRT) nodoChiediStatoRPT nodoChiediCopiaRT (OLD_NM3-12H)
    ...
    Scenario: NM3 flow KO, FLOW con PA Old e PSP vp1: verify -> paaVerificaRPT -> activate con stazione irraggiungibile con KO response -> paaAttivaRPT  nodoInviaRPT spo+ -> paaInviaRT+ (subito, senza job paInviaRT) nodoChiediStatoRPT nodoChiediCopiaRT (OLD_NM3-13H)
        Given from body with datatable horizontal verifyPaymentNoticeBody_noOptional initial XML verifyPaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                  | noticeNumber |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 305#iuv#     |
        And from body with datatable vertical paaVerificaRPT_noOptional initial XML paaVerificaRPT
        ...
        Given from body with datatable horizontal activatePaymentNoticeBody_full initial XML activatePaymentNotice
            | idPSP | idBrokerPSP | idChannel                    | password   | fiscalCode                  | noticeNumber            | amount |
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 099456789012345678      | 10.00  |
        And from body with datatable horizontal paaAttivaRPT_full initial XML paaAttivaRPT
            | esito | importoSingoloVersamento |
            | OK    | 10.00                    |
        And EC replies to nodo-dei-pagamenti with the paaAttivaRPT
