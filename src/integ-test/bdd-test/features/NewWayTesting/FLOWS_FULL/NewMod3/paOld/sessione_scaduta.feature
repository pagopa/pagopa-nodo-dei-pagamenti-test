Feature: NM3 flows PA Old sessione scaduta
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PAOLD @NM3PAOLDSESSCADUTA @NM3PAOLDSESSCADUTA_FULL_7
    Scenario: NM3 FLOW con PA Old e PSP vp1: activate -> paaAttivaRPT (scadenza sessione)  mod3cancelV1  nodoInviaRPT -> paaInviaRT- BIZ- -> activate (OLD_NM3-10H)
        Given from body with datatable horizontal activatePaymentNoticeBody_with_expiration_full initial XML activatePaymentNotice
            ...
            | #psp# | #psp#       | #canale_ATTIVATO_PRESSO_PSP# | #password# | #creditor_institution_code# | 305#iuv#     | 10.00  | 2000           |
        When psp sends SOAP activatePaymentNotice to nodo-dei-pagamenti
        Then check outcome is OK of activatePaymentNotice response
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | PA_FISCAL_CODE        | $activatePaymentNotice.fiscalCode                                                                                                                                                       |
            | NOTICE_ID             | $activatePaymentNotice.noticeNumber                                                                                                                                                     |
            | STATUS                | PAYING,CANCELLED_NORPT,CANCELLED,PAYING                                                                                                                                                 |
            | INSERTED_TIMESTAMP    | NotNone                                                                                                                                                                                 |
            | CREDITOR_REFERENCE_ID | 05$iuv                                                                                                                                                                                  |
            | PAYMENT_TOKEN         | $activatePaymentNotice_1Response.paymentToken,$activatePaymentNotice_1Response.paymentToken,$activatePaymentNotice_1Response.paymentToken,$activatePaymentNotice_2Response.paymentToken |
            | INSERTED_BY           | activatePaymentNotice,mod3CancelV1,nodoInviaRPT,activatePaymentNotice                                                                                                                   |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | FK_POSITION_PAYMENT   | NotNone                                                                                     |
            | INSERTED_BY           | activatePaymentNotice                                                                       |
            | UPDATED_BY            | nodoInviaRPT,activatePaymentNotice                                                          |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS_SNAPSHOT retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                        |
            | NOTICE_ID      | $activatePaymentNotice.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNotice.fiscalCode   |
        ...
        And from $paaInviaRTReq.identificativoStazioneIntermediarioPA xml check value #id_station_old_invio_rt_ist# in position 0
        And from $paaInviaRTReq.identificativoUnivocoVersamento xml check value 05$iuv in position 0
        And from $paaInviaRTReq.codiceContestoPagamento xml check value $activatePaymentNotice_1Response.paymentToken in position 0
        And from $paaInviaRTReq.rt xml check value NotNone in position 0
        # paaInviaRT RESP
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            | where_keys         | where_values                                  |
    ...
    @ALL @FLOW @FLOW_FULL @NM3 @NM3PAOLD @NM3PAOLDSESSCADUTA @NM3PAOLDSESSCADUTA_FULL_8
    Scenario: NM3 FLOW con PA Old e PSP vp1: verify -> activate -> paaAttivaRPT  -> nodoInviaRPT (scadenza sessione) -> mod3cancelV1 -> paaInviaRT- -> verify (OLD_NM3-11N)
