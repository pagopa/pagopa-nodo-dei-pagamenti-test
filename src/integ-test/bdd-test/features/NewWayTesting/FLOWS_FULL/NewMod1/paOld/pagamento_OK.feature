Feature: NMU flows PA Old con pagamento OK
  ...
  @ALL @FLOW @FLOW_FULL @NMU @NMUPAOLD @NMUPAOLDPAGOK @NMUPAOLDPAGOK_FULL_23
  Scenario: NMU flow OK, FLOW con PA Old vp1 e PSP vp2: checkPosition con 1 nav, activateV2 -> paaAttivaRPT, nodoInviaRPT -> closeV2+ -> pspNotify, upd idempotencyKey with None spoV2+ ok -> 1 record for the table IDEMPOTENCY_CACHE for spoV2 -> paSendRT+, e SPRv2+ (OLD_NMU-167)
    Given from body with datatable horizontal checkPositionBody initial JSON checkPosition
      | fiscalCode                  | noticeNumber |
      | #creditor_institution_code# | 312#iuv#     |
    When WISP sends rest POST checkPosition_json to nodo-dei-pagamenti
