Feature: NMU flows con PA Old sessione scaduta
    ...
    @ALL @FLOW @FLOW_FULL @NMU @NMUPAOLD @NMUPAOLDSESSCAD @NMUPAOLDSESSCAD_FULL_1
    Scenario: NMU flow sessione scaduta, FLOW con PA Old e PSP vp1, Caso di pagamento semplice per scadenza sessione prima della closeV2: checkPosition con 1 nav, activateV2 -> paaAttivaRPT, nodoInviaRPT, (scadenza sessione), mod3cancelV1 -> paaInviaRT-, BIZ-, closeV2+ con resp KO perché token scaduto (NMU-6)
        ...
        Given from body with datatable horizontal activatePaymentNoticeV2Body_with_expiration_full initial XML activatePaymentNoticeV2
            ...
            | #pspEcommerce# | #brokerEcommerce# | #canaleEcommerce# | #password# | #creditor_institution_code# | 305$iuv      | 2000           | 10.00  |
    ...
    @ALL @FLOW @FLOW_FULL @NMU @NMUPAOLD @NMUPAOLDSESSCAD @NMUPAOLDSESSCAD_FULL_3 @after
    Scenario: NMU flow sessione scaduta, FLOW con PA Old e PSP vp2, Caso di pagamento semplice per scadenza sessione dopo la closeV2: checkPosition con 1 nav, activateV2 -> paaAttivaRPT, nodoInviaRPT, closeV2+ e pspNotifyV2 malformata con outcome OO, mod3cancelV1 -> paaInviaRT- (OLD_NMU-213)
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            ...
            | PA_FISCAL_CODE        | $activatePaymentNoticeV2.fiscalCode                                                     |
            | NOTICE_ID             | $activatePaymentNoticeV2.noticeNumber                                                   |
            | STATUS                | PAYING,PAYING_RPT,PAYMENT_RESERVED,PAYMENT_SENT,PAYMENT_UNKNOWN                         |
            | INSERTED_TIMESTAMP    | NotNone                                                                                 |
            | CREDITOR_REFERENCE_ID | 12$iuv                                                                                  |
            | PAYMENT_TOKEN         | $activatePaymentNoticeV2Response.paymentToken                                           |
            | INSERTED_BY           | activatePaymentNoticeV2,nodoInviaRPT,closePayment-v2,closePayment-v2,pspNotifyPaymentV2 |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table POSITION_PAYMENT_STATUS retrived by the query on db nodo_online with where datatable horizontal
            | where_keys     | where_values                          |
            | NOTICE_ID      | $activatePaymentNoticeV2.noticeNumber |
            | PA_FISCAL_CODE | $activatePaymentNoticeV2.fiscalCode   |
