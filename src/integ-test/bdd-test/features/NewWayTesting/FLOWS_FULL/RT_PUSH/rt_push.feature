Feature: RT PUSH flow
    ...
    Scenario: RT push, FLOW con PA New e PSP che utilizza: RPT e RT con MBD -> nodoInviaCarrelloRPT -> pspInviaCarrelloRPT, nodoInviaRT  BIZ+ (OLD_RTPush-1B)
        ...
        And from $nodoInviaCarrelloRPTReq.identificativoIntermediarioPA xml check value #creditor_institution_code# in position 0
        And from $nodoInviaCarrelloRPTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
        ...
        And from $nodoInviaCarrelloRPTReq.identificativoCanale xml check value #canale_IMMEDIATO_MULTIBENEFICIARIO# in position 0
        ...
        And from $pspInviaCarrelloRPTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
    ...
    @ALL @FLOW @FLOW_FULL @RT_PUSH @RT_PUSH_6
    Scenario: RT push, FLOW con PA New e PSP che utilizza: RPT e RT con codiceEsitoPagamento 1 NON PAGATO -> nodoInviaRPT -> pspInviaRPT, nodoInviaRT  BIZ- (OLD_RTPush-7B)
        Given RPT generation RPT_generation with datatable vertical
            | identificativoDominio             | #creditor_institution_code_old# |
            | identificativoStazioneRichiedente | #id_station_old#                |
            | dataOraMessaggioRichiesta         | #timedate#                      |
    ...
    @ALL @FLOW @FLOW_FULL @RT_PUSH @RT_PUSH_7
    Scenario: RT push, FLOW con PA New e PSP che utilizza Importo diffrente tra MB e RT:  nodoInviaRPT -> pspInviaRPT, nodoInviaRT -> faultCode PPT_SEMANTICA (OLD_Validazione-1C)
        Given MB generation MBD_generation with datatable vertical
            | CodiceFiscale | #creditor_institution_code#                  |
            | Denominazione | #psp#                                        |
            | IUBD          | #iubd#                                       |
            ...
            | TipoBollo     | 01                                           |
            | DigestValue   | wHpFSLCGZjIvNSXxqtGbxg7275t446DRTk5ZrsdUQ6E= |
        And RPT generation RPT_generation_with_MBD_full with datatable vertical
            | identificativoDominio             | #creditor_institution_code# |
            | identificativoStazioneRichiedente | #id_station#                |
            | dataOraMessaggioRichiesta         | #timedate#                  |
            | dataEsecuzionePagamento           | #date#                      |
        ...
        When RT generation RT_generation_with_MBD with datatable vertical
            | identificativoDominio             | #creditor_institution_code# |
            | identificativoStazioneRichiedente | #id_station#                |
            | dataOraMessaggioRicevuta          | #timedate#                  |
            | importoTotalePagato               | 15.00                       |
            | identificativoUnivocoVersamento   | $iuv                        |
            | identificativoUnivocoRiscossione  | $iuv                        |
            | CodiceContestoPagamento           | $ccp                        |
            | codiceEsitoPagamento              | 0                           |
            | singoloImportoPagato              | 15.00                       |
            | testoAllegato                     | $bollo                      |
        And from body with datatable vertical nodoInviaRPTBody_full initial XML nodoInviaRPT
            ...
            | identificativoCanale                  | #canaleRtPush#              |
            | rpt                                   | $rptAttachment              |
        Then from body with datatable horizontal pspInviaRPT initial XML pspInviaRPT
        ...
        And from body with datatable vertical nodoInviaRTBody_noOptional initial XML nodoInviaRT
            ...
            | forzaControlloSegno             | 1                           |
            | rt                              | $rtAttachment               |
        And from body with datatable horizontal paaInviaRT initial XML paaInviaRT
            | esito |
            | OK    |
        Given EC replies to nodo-dei-pagamenti with the paaInviaRT
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column      | value                                                                                                       |
            | ID          | NotNone                                                                                                     |
            | ID_SESSIONE | $sessionToken,$sessionToken,$sessionToken,$sessionToken,NotNone,NotNone                                     |
            | STATO       | RPT_RICEVUTA_NODO,RPT_ACCETTATA_NODO,RPT_INVIATA_A_PSP,RPT_ACCETTATA_PSP,RT_RICEVUTA_NODO,RT_RIFIUTATA_NODO |
            | INSERTED_BY | nodoInviaRPT,nodoInviaRPT,nodoInviaRPT,pspInviaRPT,nodoInviaRT,nodoInviaRT                                  |
        And checks all values by $dict_fields_values_expected of the record for each columns $list_columns of the table STATI_RPT retrived by the query on db nodo_online with where datatable horizontal
            | where_keys | where_values              |
            | IUV        | $iuv                      |
            | ORDER BY   | INSERTED_TIMESTAMP,ID ASC |
        ...
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            ...
            | ORDER BY                  | DATA_ORA_EVENTO ASC |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key nodoInviaRPTReq
        And from $nodoInviaRPTReq.identificativoIntermediarioPA xml check value #creditor_institution_code# in position 0
        And from $nodoInviaRPTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
        And from $nodoInviaRPTReq.identificativoStazioneIntermediarioPA xml check value #id_station# in position 0
        And from $nodoInviaRPTReq.identificativoUnivocoVersamento xml check value $iuv in position 0
        And from $nodoInviaRPTReq.codiceContestoPagamento xml check value $ccp in position 0
        And from $nodoInviaRPTReq.password xml check value #password# in position 0
        And from $nodoInviaRPTReq.identificativoPSP xml check value #psp# in position 0
        ...
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            ...
            | ORDER BY                  | DATA_ORA_EVENTO ASC |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key pspInviaRPTReq
        And from $pspInviaRPTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
        And from $pspInviaRPTReq.identificativoPSP xml check value #psp# in position 0
        And from $pspInviaRPTReq.identificativoIntermediarioPSP xml check value #psp# in position 0
        And from $pspInviaRPTReq.identificativoCanale xml check value #canaleRtPush# in position 0
        And from $pspInviaRPTReq.modelloPagamento xml check value 1 in position 0
        ...
        And from $nodoInviaRTReq.password xml check value #password# in position 0
        And from $nodoInviaRTReq.identificativoPSP xml check value #id_broker_psp# in position 0
        And from $nodoInviaRTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
        And from $nodoInviaRTReq.identificativoUnivocoVersamento xml check value $iuv in position 0
        And from $nodoInviaRTReq.codiceContestoPagamento xml check value $ccp in position 0
        And from $nodoInviaRTReq.forzaControlloSegno xml check value 1 in position 0
        And from $nodoInviaRTReq.rt xml check value NotNone in position 0
        ...
        And execution query to get value result_query on the table RE, with the columns PAYLOAD with db name re with where datatable horizontal
            ...
            | ORDER BY                  | DATA_ORA_EVENTO ASC |
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key nodoInviaRTResp
        And from $nodoInviaRTResp.esito xml check value KO in position 0
    ...
    @ALL @FLOW @FLOW_FULL @RT_PUSH @RT_PUSH_8
    Scenario: RT push, FLOW con PA New e PSP che utilizza il canale RT_PUSH a 'N':  nodoInviaRPT -> pspInviaRPT, nodoInviaRT -> faultCode PPT_SEMANTICA (OLD_Validazione-3C)
        Given MB generation MBD_generation with datatable vertical
            | CodiceFiscale | #creditor_institution_code#                  |
            | Denominazione | #psp#                                        |
            | IUBD          | #iubd#                                       |
        ...
        And generate list columns list_columns and dict fields values expected dict_fields_values_expected for query checks all values with datatable horizontal
            | column      | value                                                                    |
            | ID          | NotNone                                                                  |
            | ID_SESSIONE | $sessionToken,$sessionToken,$sessionToken,$sessionToken                  |
        ...
        And through the query result_query retrieve xml PAYLOAD at position 0 and save it under the key nodoInviaRTReq
        And from $nodoInviaRTReq.identificativoIntermediarioPSP xml check value #psp# in position 0
        And from $nodoInviaRTReq.identificativoCanale xml check value #canaleRtPull# in position 0
        And from $nodoInviaRTReq.password xml check value #password# in position 0
        And from $nodoInviaRTReq.identificativoPSP xml check value #id_broker_psp# in position 0
        And from $nodoInviaRTReq.identificativoDominio xml check value #creditor_institution_code# in position 0
        And from $nodoInviaRTReq.identificativoUnivocoVersamento xml check value $iuv in position 0
    ...
    @ALL @FLOW @FLOW_FULL @RT_PUSH @RT_PUSH_9
    Scenario: RT push, FLOW con PA New e PSP con ordine pagamento RPT e RT diverso: nodoInviaRPT -> pspInviaRPT, nodoInviaRT -> faultCode PPT_SEMANTICA (OLD_Validazione-7C)
        Given MB generation MBD_generation with datatable vertical
            | CodiceFiscale | #creditor_institution_code#                  |
            | Denominazione | #psp#                                        |
            | IUBD          | #iubd#                                       |
        ...
        And RPT generation RPT_generation_with_IBAN_MBD with datatable vertical
            ...
            | codiceContestoPagamento           | #ccp#                       |
            | importoSingoloVersamento          | 5.00                        |
        And RT generation RT_generation_with_IBAN_MBD_inv_order with datatable vertical
            | identificativoDominio             | #creditor_institution_code# |
            | identificativoStazioneRichiedente | #id_station#                |
            | dataOraMessaggioRicevuta          | #timedate#                  |
            | importoTotalePagato               | 10.00                       |
