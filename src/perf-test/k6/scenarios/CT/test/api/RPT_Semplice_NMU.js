...
export const RPT_Semplice_NMU_Trend = new Trend('RPT_Semplice_NMU');
...
export function RPT_Semplice_NMU(baseUrl,rndAnagPaNew,paymentToken, creditorReferenceId, importoTotaleDaVersare) {
 
  //console.debug("paymToken="+paymentToken); 
  //console.debug("creditorReferenceId="+creditorReferenceId); 
  console.debug("RPT_Semplice_NMU PA "+ rndAnagPaNew.PA);
  console.debug("RPT_Semplice_NMU importoTotaleDaVersare "+ importoTotaleDaVersare);
 ...
 const res = http.post(
  ...
  console.debug("RPT_Semplice_NMU RES");
  console.debug(JSON.stringify(res));
  
   RPT_Semplice_NMU_Trend.add(res.timings.duration);
   All_Trend.add(res.timings.duration);

   check(res, {
 	':over_sla300': (r) => r.timings.duration >300,
   },
   { RPT_Semplice_NMU: 'over_sla300', ALL:'over_sla300' }
   );
   
   check(res, {
 	'RPT_Semplice_NMU:over_sla400': (r) => r.timings.duration >400,
   },
   { RPT_Semplice_NMU: 'over_sla400', ALL:'over_sla400' }
   );
   
   check(res, {
 	'RPT_Semplice_NMU:over_sla500 ': (r) => r.timings.duration >500,
   },
   { RPT_Semplice_NMU: 'over_sla500', ALL:'over_sla500' }
   );
   
   check(res, {
 	'RPT_Semplice_NMU:over_sla600': (r) => r.timings.duration >600,
   },
   { RPT_Semplice_NMU: 'over_sla600', ALL:'over_sla600' }
   );
   
   check(res, {
 	'RPT_Semplice_NMU:over_sla800': (r) => r.timings.duration >800,
   },
   { RPT_Semplice_NMU: 'over_sla800', ALL:'over_sla800' }
   );
   
   check(res, {
 	'RPT_Semplice_NMU:over_sla1000': (r) => r.timings.duration >1000,
   },
   { RPT_Semplice_NMU: 'over_sla1000', ALL:'over_sla1000' }
   );
 ...
 // console.debug("RPTSempliceN3 RESPONSE----------------"+res.body);
   ...
   check(
    res,
    {
	  ...
	  'RPT_Semplice_NMU:ok_rate': (r) => outcome == 'OK',
    },
    { RPT_Semplice_NMU: 'ok_rate', ALL:'ok_rate' }
	);
  ...
  if(check(
    res,
    {
	  ...
	  'RPT_Semplice_NMU:ko_rate': (r) => outcome !== 'OK',
    },
    { RPT_Semplice_NMU: 'ko_rate', ALL:'ko_rate' }
  )){
